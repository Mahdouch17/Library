...
app.post("/edit/:id", (req, res) => {
  ...
  const sql =
    "UPDATE Books b JOIN Authors a ON b.authorId = a.id JOIN Editions e ON b.editionId = e.id JOIN Categories c ON b.categoryId = c.id SET (b.title, b.language, a.authorName, e.editionName, c.categoryName) = (?,?,?,?,?) WHERE b.id = ?";
  db.run(sql, book, (err) => {
    if (err) {
      return console.error(err.message);
    }
